...
{ 	int choice, last_game;
	...
	play_the_game();
	 ...
	 update_player_data();
	 printf("\n Thanks for playing noob! Bye. \n");
	 return -1;
	 }
	 //This function reads the player data for the current pid
	 // from the file. It returns -1 if it is unable to find player
	 // data for the current pid.
	 int get_player_data() {
	 int fd, pid, read_bytes;
	 struct user entry;

	 pid= getpid();

	 fd = open(DATAFILE, O_RDONLY);
	 if(fd==-1) //Cant open the file , maybe it doesnt exist
	 return -1;
	 read_bytes = read(fd, &entry, sizeof(struct user)); //Read the first chunk
	 while(entry.pid != pid && read_bytes >0) { //Loop until proper pid is found.
	 ...
	 // This is the new user registration funcion.
	 ...
	 fd = open(DATAFILE, O_WRONLY|O_CREAT|O_APPEND, S_IREAD|S_IWRITE);
	 if(fd == -1)
	 char const *fatal = "in register_new_player() while opening file";
	 ...
	 if(fd == -1)
	 char const *fatal = "in update_player_data() while opening file";
	 read(fd , &read_pid, 4);
	 while(read_pid != player.pid)
	 {
	 ...
	 void show_highscore()
	 {
	  ...
	  fd = open(DATAFILE, O_RDONLY);
	  if(fd == -1)
	  char const *fatal = "in show_highscore() while opening file ";
	  ...
	  //s function simply awards the jackpot for he pick a number game.
	  void jackpot()
	  {
	  	printf("*+*+*+*+*+*+JACKPOT+*+*+*+*+*+*+\n");
	  ...
	  *name_ptr++ = 0;
	  ...
	  // It expects a message the display, a pointer to the cards array,
	...
	//This function input wagers for both the no match dealer and
  ...
  // It returns -1 if the player doesnt have enough credits.
  int pick_a_number()
  {
  ...
  if(player.credits < 10 )
  {
   ...
   //Tis is the no match dealer game.
   // It returns -1 if player has 0 credits.
   int dealer_no_match() {
   int i,j,numbers[16], wager = -1, match = -1;
   printf("\n::::::: No Match Dealer :::::::\n");
   printf("In this game, you can wager up to al of your credits. \n");
   printf(" The dealer will deal out of 16 random numbers between 0 and 99.\n");
   ...
   printf("******* Find the Ace *******\n");
   printf("In this game, you can wager upto all of your credits. \n");
   ...
   char const *print_cards = "Dealing cards";
   pick = -1;
   while((pick <1) || (pick > 3)) {
   ...
   i++;
   cards[i] = 'Q';
   char const *printcards = "Revealing a queen";
   invalid_choice = 1;
   while(invalid_choice) {
	...
	if(ace == i)
	 ...
	 char const *printcar = "End result";
	 if(pick == ace) {
